...
> 依据：需求文档 + 调研报告（docs/research_report.md）
...
5. 创新亮点可展示（详见 2.3 创新模块）：知识图谱推理、RAG 溯源、可解释性等差异化能力全部可演示。
...
创新优势源于调研报告（`docs/research_report.md`）的差异化结论：

| 依据 | 结论 |
|------|------|
| 摘要 | 核心壁垒是"知识图谱 + RAG"而非识别 |
| 4.5 竞品对比总表 | "多模态混合输入 + 知识图谱推理 + RAG 溯源问答 + 可解释性"四项组合能力，竞品矩阵中无任何一款具备，是核心差异化 |
| 4.6 SWOT 优势 | 差异化定位（识别 + 图谱 + RAG + 可解释）；十八反/十九畏安全数据；知识联动防幻觉；轻量可部署、可复现 |
| 5.2 功能矩阵 | 独占 3 项（知识图谱实体联动、RAG 溯源标注、识别可解释性关注区域）；领先 9 项（多模态混合输入、药性解析自动输出、方剂推荐、配伍禁忌、图谱可视化、多轮 AI 问答、风险提示、Web 演示 + REST API、相似药材对比） |
...
flowchart LR
    accTitle: 创新模块三层划分
    accDescr: 创新模块分为核心创新、体验创新、安全创新三层，与调研报告差异化结论对齐

    core["核心创新（壁垒型）<br/>知识图谱推理 / RAG 溯源 / 可解释性<br/>IN1-IN3"]
    exp["体验创新（闭环型）<br/>多模态混合 / 一体化闭环 / 方剂推荐<br/>IN4-IN9"]
    safety["安全创新（信任型）<br/>置信度提示 / 风险警示 / 本地降级<br/>IN10-IN12"]
    accept["验收挂钩<br/>交付物 2/3 + 验收项 3/5"]
...
| 编号 | 创新模块 | 所属（模块 / 计划条目） | 竞品对比 | 创新分层 | 优先级 | 排期 | 验收挂钩 |
|:----:|---------|:---------------------:|:-------:|:-------:|:-----:|:----:|---------|
| IN1 | 知识图谱实体联动（药-效-病-方-忌网络，含十八反/十九畏） | B / 3.1 | 独占（4.5/5.2） | 核心 | P0 | D3-D4 | 交付物 2、验收项 4 |
| IN2 | RAG 溯源问答（知识库依据标注） | C、D / 4.2、5.3 | 独占（4.5/5.2） | 核心 | P0 | D3-D6 | 交付物 3、验收项 4 |
| IN3 | 识别可解释性（关注区域 Grad-CAM） | D / 5.3 | 独占（5.2） | 核心 | P1 | D5-D6 | 交付物 3 |
| IN4 | 多模态混合输入（图片 + 文字混合识别/提问） | A、C / 2.1、4.2 | 领先（4.5） | 体验 | P0 | D3-D4 | 交付物 2 |
| IN5 | 识别—解析—推荐一体化闭环（拍图→药性→方剂） | A、B、C / 2.1、3.2、4.1 | 领先（5.2） | 体验 | P0 | D3-D4 | 交付物 2 |
| IN6 | 方剂推荐 + 配伍风险标注 | B / 3.2 | 领先（5.2） | 体验 | P0 | D3-D4 | 交付物 2、验收项 3 |
| IN7 | 相似药材提示（外观差异与鉴别要点） | A / 2.2 | 领先（5.2） | 体验 | P1 | D5-D6 | 交付物 3 |
| IN8 | 图谱可视化交互 | B / 3.3 | 领先（5.2） | 体验 | P0 | D3-D4 | 交付物 2 |
| IN9 | 文本特性检索反查（按性味/功效找药） | B / 3.3 | 领先（5.2） | 体验 | P0 | D3-D4 | 交付物 2 |
| IN10 | 置信度提示 + 低置信度人工复核引导 | A、D / 2.1、5.1 | 领先（5.2） | 安全 | P0 | D3-D4 | 验收项 3 |
| IN11 | 医疗风险提示 + 毒性药材强制警示 | D / 5.2 | 领先（5.2） | 安全 | P0 | D3-D4 | 验收项 3 |
| IN12 | 无 Key 本地降级（离线可演示） | E / 6.2 | 领先（5.2） | 安全 | P0 | D3-D4 | 验收项 5 |

> **说明**：IN1-IN12 覆盖调研报告 4.5/5.2 全部"独占 / 领先"项；每项均挂钩计划 3.1 排期与 5 节验收标准，可落地、可验收。P1 项（IN3、IN7）按缓冲策略在进度落后时可裁剪，不挤压验收时间（见 4.2-1）。
...
| D5-D6 | 创新增强 | 相似药材提示（输出外观差异与鉴别要点）；RAG 溯源来源展示强化（创新模块 IN2/IN3/IN7，详见 2.3） | 差异化功能演示通过（交付物 3） |
